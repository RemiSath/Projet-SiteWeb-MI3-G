<!DOCTYPE html>
<html lang="fr">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="styles.css">
    <link rel="icon" href="Images/Among_Us.png">
    <title>Imposteur</title>
</head>

<body>
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Playwrite+AT:ital,wght@0,100..400;1,100..400&display=swap"
        rel="stylesheet">
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Pacifico&display=swap" rel="stylesheet">
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Annie+Use+Your+Telescope&display=swap" rel="stylesheet">

    <header class="navbar">
        <a href="page-d'accueil.html" class="accueil">IMPOSTEUR</a>
        <div class="navliens">
            <div class="menu">
                <a>Réservation</a>
                <div class="infos">
                    <a href="reserver.html">Réserver une table</a>
                    <a href="commander.html">Commander</a>
                </div>
            </div>
            <div class="menu">
                <a href="Notation.html">Notation</a>
            </div>
            <div class="menu">
                <a>Compte</a>
                <div class="infos">
                    <a href="profil.html">Voir Profil</a>
                    <a href="connexion.html">Connexion</a>
                    <a href="inscription.php">Inscription</a>
                </div>
            </div>
            <div class="menu">
                <a>Services</a>
                <div class="infos">
                    <a href="commandes.html">Commandes</a>
                    <a href="livraison.html">Livraison</a>
                </div>
            </div>
            <div class="menu">
                <a href="Admin.html">Admin</a>
            </div>
            <input type="text" id="searchInput2" placeholder="Rechercher nos produits ..." autocomplete="off">
        </div>
    </header>

    <div class="sitedescription">
        <h1>Bienvenue chez l'<span class="nomsite">IMPOSTEUR</span></h1>
        <p class="textedescription"><span class="nomsite">IMPOSTEUR</span> est une maison dédiée à l'illusion.
            Ici, rien n’est tout à fait ce qu'il paraît. Chaque création détourne les apparences, brouille les repères
            et joue avec vos sens.

            Nos plats prennent la forme d'objets du quotidien, de mets salés ou de matières inattendues…
            mais tout est pâtisserie.

            Textures, volumes, reflets, détails : chaque trompe-l'œil est pensé comme une œuvre comestible.
            À table, l’œil doute. La main hésite. Le palais confirme.

            Bienvenue dans l’art de la confusion délicieuse.</p>
    </div>

    <div class="contenu">
        <div class="villes">
            <a href="Paris.html" class="a1 a10">Paris</a>
            <a href="Argenteuil.html" class="a1 a11">Argenteuil</a>
            <a href="Cergy.html" class="a1 a12">Cergy</a>
        </div>

        <div class="villesimg">
            <div class="villedescription">
                <div class="img paris"></div>
                <div class="infosville">
                    <h3>Paris</h3>
                    <p>67 rue Pasteur</p>
                </div>
            </div>
            <div class="villedescription">
                <div class="img argenteuil"></div>
                <div class="infosville">
                    <h3>Argenteuil</h3>
                    <p>101 rue de Malcouture</p>
                </div>
            </div>
            <div class="villedescription">
                <div class="img cergy"></div>
                <div class="infosville">
                    <h3>Cergy</h3>
                    <p>3 rue du Parc</p>
                </div>
            </div>
        </div>


        <div class="platsjour">
            <h2>Plats du jour</h2>
        </div>

        <div class="plats paris">
            <div class="platdescription">
                <div class="plat plat1"></div>
                <div class="description">
                    <h2>Citron</h2>
                    <p>Prix : 5,50€</p>
                </div>
            </div>
            <div class="platdescription">
                <div class="plat plat2"></div>
                <div class="description">
                    <h2>Pomme</h2>
                    <p>Prix : 7,00€</p>
                </div>
            </div>
            <div class="platdescription">
                <div class="plat plat3"></div>
                <div class="description">
                    <h2>Poire</h2>
                    <p>Prix : 7,20€</p>
                </div>
            </div>
        </div>

        <div class="plats argenteuil">
            <div class="platdescription">
                <div class="plat plat4"></div>
                <div class="description">
                    <h2>Pomme</h2>
                    <p>Prix : 7,00€</p>
                </div>
            </div>
            <div class="platdescription">
                <div class="plat plat5"></div>
                <div class="description">
                    <h2>Poire</h2>
                    <p>Prix : 7,20€</p>
                </div>
            </div>
            <div class="platdescription">
                <div class="plat plat6"></div>
                <div class="description">
                    <h2>Mandarine</h2>
                    <p>Prix : 6,90€</p>
                </div>
            </div>
        </div>

        <div class="plats cergy">
            <div class="platdescription">
                <div class="plat plat7"></div>
                <div class="description">
                    <h2>Mandarine</h2>
                    <p>Prix : 6,90€</p>
                </div>
            </div>
            <div class="platdescription">
                <div class="plat plat8"></div>
                <div class="description">
                    <h2>Mangue</h2>
                    <p>Prix : 7,50€</p>
                </div>
            </div>
            <div class="platdescription">
                <div class="plat plat9"></div>
                <div class="description">
                    <h2>Citron</h2>
                    <p>Prix : 6,50€</p>
                </div>
            </div>
        </div>
    </div>

    <footer class="footer">
        <p>📞 Téléphone : 07 67 01 02 03</p>
        <p>✉ Email : imposturecontact@gmail.com</p>
        <p>Horaires : Lundi - Vendredi 10h-21h | Samedi - Dimanche 12h-18h</p>
    </footer>

</body>

</html>